        ...
        <header className={`sticky top-0 z-50 transition-all duration-300 font-montserrat ${isScrolled ? 'bg-white/95 backdrop-blur-md shadow-md' : 'bg-white/80'}`}>
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                <div className={`flex items-center justify-between transition-all duration-300 ${isScrolled ? 'h-20' : 'h-24'}`}>
                    ...
                    {/* --- RIGHT SIDE: QUOTE BUTTON + MENU ICON --- */}
                ...
                <div className="flex flex-col items-start space-y-5 px-8 py-8">
                    {navLinks.map(link => (
                        <NavLink
                            ...
                            className={({ isActive }) => `text-lg ${navLinkClasses} ${isActive ? activeNavLinkClasses : ''}`}
                        >
                            {link.name}
                        </NavLink>
                    ))}

                    {/* --- ADD "GET A QUOTE" BUTTON INSIDE DROPDOWN --- */}
                    <Link 
                        to="/booking"
                        onClick={() => setIsMenuOpen(false)}
